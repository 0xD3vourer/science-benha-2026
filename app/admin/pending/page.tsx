...
export default function PendingPage() {
  ...
  // فحص التكرار بالاسم + القسم
  const isDuplicate = (name: string, section: string) =>
    existingMembers.some(
      (member) => member.name === name && member.section === section
    );

  // فحص لو الإيميل ده مسجَّل قبل كده
  ...
  const handleApprove = async (request: any) => {
    // فحص التكرار في الاسم/القسم
    if (isDuplicate(request.name, request.section)) {
      toast.error(
        `⚠️ الاسم "${request.name}" موجود بالفعل في قسم "${request.section}"`
      );
      return;
    }

    // فحص الإيميل (مهم — لازم email يكون موجود في الطلب)
    const email = (request.email || '').toString().trim().toLowerCase();
    if (!email) {
      toast.error(
        '⚠️ الطلب ده مفيهوش إيميل — مش هينفع اليوزر يعمل login من غير إيميل'
      );
      return;
    }
    if (await isEmailDuplicate(email)) {
      toast.error(`⚠️ الإيميل "${email}" مسجَّل قبل كده`);
      return;
    }

    setProcessing(request.id);
    try {
      // 1. حفظ العضو في graduates مع الإيميل + status
      await addDoc(collection(db, 'graduates'), {
        name: request.name,
        section: request.section || 'غير محدد',
        email, // ✅ كده اليوزر هيقدر يعمل login
        status: 'approved', // ✅ source of truth واحد
        photoUrl: request.photoUrl || '',
        approvedAt: serverTimestamp(),
      });

      // 2. تحديث Google Sheets + بعت إيميل الموافقة لليوزر
      //    (action=approve في الـ Apps Script يعمل sheet update + يبعت إيميل تلقائياً)
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            الطلبات الجديدة من Google Forms تظهر هنا تلقائياً
          </p>
        </div>
      </div>
      ...
      {pendingRequests.length === 0 ? (
        ...
        <div className="space-y-3">
          {pendingRequests.map((request, idx) => {
            ...
            const missingEmail = !request.email;
            const blocked = duplicateExists || missingEmail;
            return (
              <motion.div
                key={request.id}
                ...
                className={`bg-black/40 backdrop-blur-sm rounded-xl p-5 border ${
                  blocked
                ...
                <div className="flex justify-between items-start flex-wrap gap-4">
                  <div className="flex-1">
                    <h3
                      className={`text-xl font-bold ${
                        blocked ? 'text-red-400' : 'text-white'
                      }`}
                    >
                      {request.name}
                    </h3>
                    ...
                    {duplicateExists && (
                      <p className="text-red-400 text-xs mt-2">
                        ⚠️ هذا الاسم موجود بالفعل في قائمة الخريجين
                      </p>
                    )}
                    {missingEmail && (
                      <p className="text-red-400 text-xs mt-2">
                        ⚠️ الطلب مفيهوش إيميل — مش هينفع يعمل login
                      </p>
                    )}
                  </div>
                  <div className="flex gap-3">
                    <button
                      ...
                      disabled={processing === request.id || blocked}
                      className={`px-5 py-2 rounded-lg font-semibold transition disabled:opacity-50 ${
                        blocked
